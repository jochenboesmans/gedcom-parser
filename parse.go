...
func main() {
	...
	fmt.Printf("interpreted family in %f second.\n", float64(familyTime)*math.Pow10(-9))
	...
	for _, personPtr := range gedcom.Persons {
		gedcomWithoutLock.Persons = append(gedcomWithoutLock.Persons, *personPtr)
	}
...
func interpretPersonRecord(gedcom *model.Gedcom, currentRecordDeepLines []*gedcomSpec.Line, currentRecordLine *gedcomSpec.Line) {
	...
	for i, line := range currentRecordDeepLines {
		if i != 0 && line.Level() != nil && *line.Level() == 0 {
			break
		}
		if line.Level() != nil && *line.Level() == 1 {
			switch *line.Tag() {
			case "NAME":
